...
- Line 73-80: Output directories for static libraries, shared libraries, and executables (only for executable and library targets)
- Line 82-83: Linker options (only for executable and library targets)
- Line 85-86: The target version (only for executable and library targets)
- Line 88-89: The target so version in case of a shared library (only for executable and library targets)
- Line 91-92: The target output name (only for executable and library targets)
- Line 94-95: The C++ standard used for compilation (only for executable and library targets)
...
Explanation:
- Line 34-46: Depending on the variable `BAREMETAL_TARGET` which we have defined in the CMakeSettings.json file, we define the variables `BAREMETAL_ARCH_CPU_OPTIONS` and `BAREMETAL_TARGET_KERNEL`, which define specific compiler options for the platform we're building for (Raspberry Pi 3, 4 or 5), and the base name of the kernel image file.
...
This makes it possible to compare board versions
...
```
File: CMakeLists.txt
195: if (PLATFORM_BAREMETAL)
196:     set(LINKER_START_GROUP -Wl,--start-group)
197:     set(LINKER_END_GROUP -Wl,--end-group)
198: endif()
199: 
200: set(LINKER_LIBRARIES ${LINK_LIBRARIES})
```
...
33:     )
34: 
35: file(GLOB_RECURSE PROJECT_SOURCES src/*.cpp src/*.S)
...
60: endif()
61: 
62: add_executable(${PROJECT_NAME} ${PROJECT_SOURCES} ${PROJECT_INCLUDES_PUBLIC} ${PROJECT_INCLUDES_PRIVATE})
63: 
64: target_link_libraries(${PROJECT_NAME} ${LINKER_START_GROUP} ${PROJECT_LIBS} ${LINKER_END_GROUP})
65: target_include_directories(${PROJECT_NAME} PRIVATE ${PROJECT_INCLUDE_DIRS_PRIVATE})
66: target_include_directories(${PROJECT_NAME} PUBLIC  ${PROJECT_INCLUDE_DIRS_PUBLIC})
67: target_compile_definitions(${PROJECT_NAME} PRIVATE
68:     $<$<COMPILE_LANGUAGE:C>:${PROJECT_COMPILE_DEFINITIONS_C_PRIVATE}>
69:     $<$<COMPILE_LANGUAGE:CXX>:${PROJECT_COMPILE_DEFINITIONS_CXX_PRIVATE}>
70:     $<$<COMPILE_LANGUAGE:ASM>:${PROJECT_COMPILE_DEFINITIONS_ASM_PRIVATE}>
71:     )
72: target_compile_definitions(${PROJECT_NAME} PUBLIC
73:     $<$<COMPILE_LANGUAGE:C>:${PROJECT_COMPILE_DEFINITIONS_C_PUBLIC}>
74:     $<$<COMPILE_LANGUAGE:CXX>:${PROJECT_COMPILE_DEFINITIONS_CXX_PUBLIC}>
75:     $<$<COMPILE_LANGUAGE:ASM>:${PROJECT_COMPILE_DEFINITIONS_ASM_PUBLIC}>
76:     )
77: target_compile_options(${PROJECT_NAME} PRIVATE
78:     $<$<COMPILE_LANGUAGE:C>:${PROJECT_COMPILE_OPTIONS_C_PRIVATE}>
79:     $<$<COMPILE_LANGUAGE:CXX>:${PROJECT_COMPILE_OPTIONS_CXX_PRIVATE}>
80:     $<$<COMPILE_LANGUAGE:ASM>:${PROJECT_COMPILE_OPTIONS_ASM_PRIVATE}>
81:     )
82: target_compile_options(${PROJECT_NAME} PUBLIC
83:     $<$<COMPILE_LANGUAGE:C>:${PROJECT_COMPILE_OPTIONS_C_PUBLIC}>
84:     $<$<COMPILE_LANGUAGE:CXX>:${PROJECT_COMPILE_OPTIONS_CXX_PUBLIC}>
85:     $<$<COMPILE_LANGUAGE:ASM>:${PROJECT_COMPILE_OPTIONS_ASM_PUBLIC}>
86:     )
87: 
88: set_property(TARGET ${PROJECT_NAME} PROPERTY CXX_STANDARD ${SUPPORTED_CPP_STANDARD})
89: 
90: list_to_string(PROJECT_LINK_OPTIONS PROJECT_LINK_OPTIONS_STRING)
91: if (NOT "${PROJECT_LINK_OPTIONS_STRING}" STREQUAL "")
92:     set_target_properties(${PROJECT_NAME} PROPERTIES LINK_FLAGS "${PROJECT_LINK_OPTIONS_STRING}")
93: endif()
94: 
95: link_directories(${LINK_DIRECTORIES})
96: set_target_properties(${PROJECT_NAME} PROPERTIES OUTPUT_NAME ${PROJECT_TARGET_NAME})
97: set_target_properties(${PROJECT_NAME} PROPERTIES ARCHIVE_OUTPUT_DIRECTORY ${OUTPUT_LIB_DIR})
98: set_target_properties(${PROJECT_NAME} PROPERTIES RUNTIME_OUTPUT_DIRECTORY ${OUTPUT_BIN_DIR})
99: 
100: show_target_properties(${PROJECT_NAME})
101: 
102: set(BAREMETAL_EXECUTABLE_TARGET ${PROJECT_NAME})
103: setup_image(${PROJECT_NAME})
```
...
- Line 39-60: Using the custom CMake function `display_list` we can now print all settings for the project.
This is only done if verbose output is requested
- Line 67-86: The compiler definitions and options are set as before
- Line 88: We can now use the `SUPPORTED_CPP_STANDARD` to set the C++ standard property on the target
- Line 95: We set the search path for standard libraries using `LINK_DIRECTORIES`
- Line 100: Using the custom CMake function `show_target_properties` we can now print all target properties.
...
1> [CMake] 
1> [CMake] -- 
1> [CMake] ## In directory: D:/Projects/RaspberryPi/baremetal.github/tutorial
...
- The `PLATFORM_BAREMETAL` definition is triggered by the main CMake file, along with the definition of `BAREMETAL_RPI_TARGET`:
...
- Then the definitions are set on the target as a property using this veriable:
...
- The first option -g is automatically added by CMake as we are building for a debug configuration.
